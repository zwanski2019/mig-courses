...
use DateTime;
use GenieAi\App\ProLabel\GenieNotice as LibsNotice;

use function PHPSTORM_META\elementType;

if (!class_exists('Getgenie\App\ProLabel\GenieRating')) {
    ...
    class GenieRating
    {
         ...
         * Get vesrion of $this
         *
         * @return \GenieAi\App\Prolabel\GenieRating
         */
        ...
        {
            add_action('wp_ajax_getgenie_rating_never_show_message', array(__CLASS__, 'never_show_message'));
            add_action('wp_ajax_getgenie_rating_ask_me_later_message', array(__CLASS__, 'ask_me_later_message'));
        }

        protected function is_current_screen_allowed($current_screen_id)
        {
            if (in_array($current_screen_id, array_merge($this->plugin_screens, array('dashboard', 'plugins', 'toplevel_page_getgenie')))) {
                return true;
            }
        ...
        public static function never_show_message()
        {
            ...
            if (empty($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'getgenie_rating')) {
                return false;
            }
        ...
        public function display_message_box()
        {
            ...
            if (!$this->duplication) {
                global $getgenie_libs_execution_container;

                if (isset($getgenie_libs_execution_container['rating'])) {
                    return;
                }
            }

            $getgenie_libs_execution_container['rating'] = __FILE__;
            ...
            if (abs($diff_interval) >= $this->days) {
                ...
                $message = "Thank you for using {$this->plugin_name} to create great content! Can you please give us a 5-star rating on WordPress to motivate us and help new users make their decision?";
                ...
                jQuery(document).ready(function ($) {
                    ...
                    $( '#" . esc_js($this->text_domain) . "_btn_already_did' ).on( 'click', function() {
                        ...
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {
                                action 	: 'getgenie_rating_never_show_message',
                                plugin_name : '" . esc_js($this->text_domain) . "',
								nonce : '" . esc_js(wp_create_nonce('getgenie_rating')) . "'
                    ...
                    $('#" . esc_js($this->text_domain) . "_btn_not_good').click(function(){
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {
                                action 	: 'getgenie_rating_ask_me_later_message',
                                plugin_name : '" . esc_js($this->text_domain) . "',
								nonce : '" . esc_js(wp_create_nonce('getgenie_rating')) . "'
                            },
                            success:function(response){
