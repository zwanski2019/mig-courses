...
if ( ! class_exists( '\GenieAi\App\ProLabel\GenieNotice' ) ) :
	...
	class GenieNotice {
		...
		public static function init() {
			add_action( 'wp_ajax_genie-notices', array( __CLASS__, 'dismiss_ajax_call' ) );
			add_action( 'admin_head', array( __CLASS__, 'enqueue_scripts' ) );
		}

		public static function dismiss_ajax_call() {

			if( empty( $_POST['nonce'] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'genie-notices' ) ){
				return false;
			}
                ...
                jQuery(document).ready(function ($) {
                    $( '.wpmet-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {
                        ...
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {
                                action 	        : 'genie-notices',
                                notice_id 		: notice_id,
                                dismissible 	: dismissible,
                                expired_time 	: expired_time,
								nonce 			: '" . esc_js( wp_create_nonce( 'genie-notices' ) ) . "'
                            },
                        });
                    });
                });
