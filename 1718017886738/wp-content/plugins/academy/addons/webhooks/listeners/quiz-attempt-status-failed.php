...
class QuizAttemptStatusFailed implements ListenersInterface {
	...
	public static function get_payload( $attempt_quiz ) {
		$data = array();
		if ( is_object( $attempt_quiz ) && 'failed' === $attempt_quiz->attempt_status ) {
			$data = Payload::get_quiz_attempt_object_data( $attempt_quiz );
		} elseif ( 'failed' === $attempt_quiz['attempt_status'] ) {
			$data = Payload::get_quiz_attempt_array_data( $attempt_quiz );
		}

		return apply_filters( 'academy_webhooks/quiz_attempt_status_failed', $data );
	}
}
