...
if ( ! class_exists( '\GenieAi\App\ProLabel\GenieBanner' ) ) :

	class GenieBanner {

		protected $script_version = '2.2.0';

		protected $key = 'getgenie_banner';
		protected $data;
		...
		public function display_content() {
			...
			foreach ( $this->data as $content ) {
				...
				if ( $content->start <= time() && time() <= $content->end ) {
					...
					if ( $this->is_correct_screen_to_show( $content->screen, $screen->id ) && class_exists( '\GenieAi\App\ProLabel\GenieNotice' ) ) {
						...
						$instance = GenieNotice::instance( 'wpmet-jhanda', $banner_unique_id )
